...
| SVC-014 | Web UI settings page | Claude | Bench Test | Medium | **Codex FINAL APPROVE 2026-07-02** (auth/CSRF/body-only, whitelist fail-closed, bounded+escaped JSON, no password in URL; P3 constant-time `str_eq_ct` fixed). `idf.py build`/`ninja` PASS + host tests PASS. Bench-ready — procedure: `engineering/10_Bench_Test_SVC-014_WebUI.md` (auth/CSRF, whitelist fail-closed, secrets not returned, password-not-in-URL, bounded body/JSON, persistence, **v5 migration `was_default` on real NVS**, provisioning lockout). Installer settings page + `POST /api/config`. Pure host-tested whitelist `components/webui/webui_settings.{h,c}` (write-whitelist EXCLUDES board_id/version/crc/provisioned/setup_password/webui_require_auth/relay state → settings can't weaken auth). Handler runs `guard_mutating` (auth+CSRF); body-only urlencoded; password never in URL; working-copy→sanitize→save→atomic swap→control_reload. GET emits installer field names (no secrets). Host tests PASS (ASan/UBSan). ESP-IDF build must run on the Mac. Awaiting Codex before bench/merge |
| SVC-015 | MQTT / CNX integration | Claude | Bench Test | Medium | Spec: `docs/40_MQTT_CNX_INTEGRATION.md`. **Phase 0: Codex FINAL APPROVE 2026-07-03 — bench-safe** (config/schema/whitelist/tests; P2 port-wrap + P3 doc fixed). `idf.py build` PASS + host tests PASS. **Phase 0 DONE (config only):** schema **v6** MQTT block (enabled/host/port/user/pass/client_id/topic_prefix/tls/allow_remote_control/command_timeout_ms); frozen `svc_config_v5_t` + **v5→v6 migration** (preserves provisioning + all fields; MQTT defaults); defaults (MQTT **off**, remote control **off**, port 1883, timeout 5000) + sanitizer (NUL-term, bool normalize, port/timeout default, topic-prefix build + scrub to `[a-zA-Z0-9_-/]`, no wildcards); Web UI POST whitelist += MQTT installer fields; `mqtt_pass` write-only (`webui_settings_is_secret()`), **never returned by GET**. Host tests +MQTT (ALL PASS ASan/UBSan); offline syntax-check clean. **No network/subscriber yet.** Needs `idf.py build` + Codex review. Next: Phase 1 telemetry publisher (`components/mqtt_bridge`), Phase 2 gated command subscriber. |
...
| SVC-025 | hal_svc100 (re-home board.c) | Claude | Review | Medium | **Phase 1 DONE 2026-07-03**: app_main now brings the board up THROUGH the HAL — `hal_install()` (registers SVC-100 driver+profile) then `hal_board_init()` (runs board_init() + de-energized safe state) replace the direct `board_init()` call. `hal_install()` declared in `hal_board.h`; `main` REQUIRES += `svc_hal hal_svc100`. Behavior parity for SVC-100 (relay_init still sets polarity/safe state). IO services still use BOARD_* directly (moved in SVC-024). Offline syntax-check app_main.c exit 0; host tests PASS. Needs `idf.py build` + bench (relay safe-off on cold/EN/OTA boot) before Done |
